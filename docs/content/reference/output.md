...
describes what users get. Pick a format with `-o`, or let twelvedata choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
twelvedata <command> -o table   # aligned columns for reading
twelvedata <command> -o jsonl   # one JSON object per line, for piping
twelvedata <command> -o json    # a single JSON array
twelvedata <command> -o csv     # spreadsheet friendly
twelvedata <command> -o tsv     # tab-separated
twelvedata <command> -o url     # just the URL column
twelvedata <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
twelvedata <command> --fields id,title,url
```
...
```bash
twelvedata <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
twelvedata <command>            # a table, because this is a terminal
twelvedata <command> | wc -l    # JSONL, because this is a pipe
```
